...
export function HomeScreen({ onAcceptConnection, onOpenSettings }: HomeScreenProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="absolute bottom-6 right-6 flex items-center gap-3 px-4 py-2 rounded-full bg-black/60 backdrop-blur-xl border border-white/10 text-xs font-mono text-gray-400 hover:bg-black/80 transition-colors cursor-help">
        <div className={`w-2 h-2 rounded-full ${status === 'CONNECTED' || status === 'IN_SESSION' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-rose-500/50'}`} />
