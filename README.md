...
The host machine captures its desktop as a media stream.

The stream is encoded and transmitted in real time using WebRTC.

A signaling server is used to establish peer-to-peer connections.

The controller machine receives and displays the live video feed.

Mouse and keyboard events are sent back to the host via a real-time control channel.
